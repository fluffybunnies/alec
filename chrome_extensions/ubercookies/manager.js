...
function onload(){
	chrome.cookies.getAll({
		...
		cookies.forEach(function(cookie){
			...
			byDomain[cookie.domain].push(cookie.name+'='+cookie.value)
		})
		Object.keys(byDomain).forEach(function(domain){
			byDomain[domain] = byDomain[domain].join('; ')
		})
		select('#output').value = JSON.stringify(byDomain,null,3)
	})
}
